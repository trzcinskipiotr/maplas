<template>
  <div>
    ...
    <MapPlace v-for="place of $store.state.placesKomootPoi" :key="place.id" :place="place"></MapPlace>
  </div>
</template>
...
@Component
export default class KomootPoiCard extends BaseComponent {

  private checkedAll: boolean = false;

  @Watch('checkedAll')
  private onCheckedAllChanged(value: boolean, oldValue: boolean) {
    for(const place of this.$store.state.placesKomootPoi) {
      if (this.checkedAll) {
