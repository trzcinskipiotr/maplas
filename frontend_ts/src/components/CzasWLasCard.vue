<template>
  <div>
    ...
    <MapPlace v-for="place of $store.state.placesCzasWLas" :key="place.id" :place="place"></MapPlace>
  </div>
</template>
...
@Component
export default class CzasWLasCard extends BaseComponent {

  private checkedAll: boolean = false;

  @Watch('checkedAll')
  private onCheckedAllChanged(value: boolean, oldValue: boolean) {
    for(const place of this.$store.state.placesCzasWLas) {
      if (this.checkedAll) {
